...
size_t spline::find_closest(double x) const
{
    ...
    size_t idx = fmax( int(it-m_x.begin())-1, 0);   // m_x[idx] <= x
    return idx;
}
...
std::vector<double> spline::solve(double y, bool ignore_extrapolation) const
{
    ...
    for(size_t i=0; i<n-1; i++) {
        ...
        for(size_t j=0; j<root.size(); j++) {
            ...
            double eps = internal::get_eps()*512.0*fmin(h,1.0);
...
void band_matrix::lu_decompose()
{
    ...
    for(int i=0; i<this->dim(); i++) {
        ...
        j_min=fmax(0,i-this->num_lower());
        j_max=fmin(this->dim()-1,i+this->num_upper());
    ...
    for(int k=0; k<this->dim(); k++) {
        i_max=fmin(this->dim()-1,k+this->num_lower());  // num_lower not a mistake!
        for(int i=k+1; i<=i_max; i++) {
            assert(this->operator()(k,k)!=0.0);
            x=-this->operator()(i,k)/this->operator()(k,k);
            this->operator()(i,k)=-x;                         // assembly part of L
            j_max=fmin(this->dim()-1,k+this->num_upper());
...
std::vector<double> band_matrix::l_solve(const std::vector<double>& b) const
{
    ...
    for(int i=0; i<this->dim(); i++) {
        sum=0;
        j_start=fmax(0,i-this->num_lower());
...
std::vector<double> band_matrix::r_solve(const std::vector<double>& b) const
{
    ...
    for(int i=this->dim()-1; i>=0; i--) {
        sum=0;
        j_stop=fmin(this->dim()-1,i+this->num_upper());
