...
color dielectric::shading(const ray& r, light& light, hit_record& hr, point3& camera_pos, hittable_list& world, int depth) {
	color L= light.p_shading(hr, camera_pos, world); // in assenza di luci sarà NaN
	...
	// probabilmente si potrebbe fare lo stesso con cos_theta_t (o il suo quadrato: discriminant)
	...
	// il dielettrico ha un colore di riflessione ks che assieme a kr modulerà la luce proveniente da wo
	color fr = ks * kr;
	...
	if (kr == 1.0) { // riflessione totale
		if (hr.front_face) {
			...
			// la luce proveniente dal raggio riflesso è Lr, ma questa va modulata innanzitutto da fr (che però è ks)
			...
			L += color(pow(c_out[0], t) * Lr[0] * ks[0], pow(c_out[1], t) * Lr[1] * ks[1], pow(c_out[2], t) * Lr[2] * ks[2]);
			//L = L / 2.0;
		}
		else {
			Lr = ray_color(reflected_ray, world, light, camera_pos, t, depth + 1);
			L += color(pow(c_in[0], t) * Lr[0] * ks[0], pow(c_in[1], t) * Lr[1] * ks[1], pow(c_in[2], t) * Lr[2] * ks[2]);
			//L = L / 2.0;
		}
	}
	else {
